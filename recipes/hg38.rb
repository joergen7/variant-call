...
hg38_smallname = "small.fa"
hg38_fullname = "hg38.tar"
...
hg38_idlarge.each { |id|
  ...
  bash "gunzip #{id}" do
    ...
    cwd hg38_dir
    not_if "#{File.exists?( "#{hg38_dir}/#{id}.fa" ) || File.exists?( "#{hg38_dir}/#{hg38_fullname}" )}"
  end
}
...
bash "hg38_concatenate_small_fa" do
  code "cat #{hg38_small.map { |id| "#{id}.fa.gz" }.join( " " )} | gunzip -c > #{hg38_smallname}"
  cwd hg38_dir
  not_if "#{File.exists?( "#{hg38_dir}/#{hg38_smallname}" ) || File.exists?( "#{hg38_dir}/#{hg38_fullname}" )}"
end

bash "hg38_tar_fa" do
  code "tar --remove-files -cf #{hg38_fullname} #{hg38_idlarge.map { |id| "#{id}.fa" }.join( " " )} #{hg38_smallname}"
  cwd hg38_dir
  not_if "#{File.exists?( "#{hg38_dir}/#{hg38_fullname}" )}"
end
